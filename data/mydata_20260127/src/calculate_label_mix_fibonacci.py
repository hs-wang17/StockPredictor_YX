...
stk_ret_13 = (all_stk_close_1300_df.pct_change(13).shift(-13)).dropna(how="all")
idx_ret_13_list, dates_list = [], []
for file in tqdm(MIN_DATA_DIR_LIST[:-13]):
    date = file.replace(".fea", "")
    stk_ret = stk_ret_13.loc[date].dropna()
    idx_weight_df = pd.read_feather(os.path.join(INDEX_WEIGHT_DIR, file))
    idx_weight = idx_weight_df[idx_weight_df["index_name"] == "ZZ1000"].set_index("stock_code")["stock_weight"]
    common_codes = stk_ret.index.intersection(idx_weight.index)
    stk_ret = stk_ret.loc[common_codes]
    idx_weight = idx_weight.loc[common_codes]
    idx_ret = (stk_ret * idx_weight).sum() / idx_weight.sum()
    idx_ret_13_list.append(idx_ret)
    dates_list.append(date)
idx_ret_13 = pd.Series(idx_ret_13_list, index=dates_list)
label_13 = stk_ret_13.sub(idx_ret_13, axis=0).dropna(how="all")

stk_ret_21 = (all_stk_close_1300_df.pct_change(21).shift(-21)).dropna(how="all")
idx_ret_21_list, dates_list = [], []
for file in tqdm(MIN_DATA_DIR_LIST[:-21]):
    date = file.replace(".fea", "")
    stk_ret = stk_ret_21.loc[date].dropna()
    idx_weight_df = pd.read_feather(os.path.join(INDEX_WEIGHT_DIR, file))
    idx_weight = idx_weight_df[idx_weight_df["index_name"] == "ZZ1000"].set_index("stock_code")["stock_weight"]
    common_codes = stk_ret.index.intersection(idx_weight.index)
    stk_ret = stk_ret.loc[common_codes]
    idx_weight = idx_weight.loc[common_codes]
    idx_ret = (stk_ret * idx_weight).sum() / idx_weight.sum()
    idx_ret_21_list.append(idx_ret)
    dates_list.append(date)
idx_ret_21 = pd.Series(idx_ret_21_list, index=dates_list)
...
label_mix.to_feather("/home/haris/raid0/shared/haris/mydata_20260127/label_mix_fibonacci.fea")
trade_date_df = pd.DataFrame({"trade_date": label_mix.index})
trade_date_df.reset_index(drop=True).to_feather("/home/haris/raid0/shared/haris/mydata_20260127/trade_date_mix_fibonacci.fea")
