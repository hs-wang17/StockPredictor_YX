...
def make_predictions_neural_network(
    ...
    result_df = pd.DataFrame({"date": dates, "stock_code": stock_codes, "prediction": predictions}).pivot(
    ...
    if use_swanlab:
        swanlab.log({"prediction/mean": np.mean(predictions), "prediction/std": np.std(predictions)})
