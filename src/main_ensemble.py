...
def run():
    ...
    for i in range(num_periods)[:2]:  # 这里保留你原来的 [:2] 限制，必要时移除
        ...
        logger.info(
            f"  Train Dates: {train_dates_list[i][0]} to {train_dates_list[i][-1]}; Length: {len(train_dates_list[i])} days"
        )
        logger.info(
        ...
        predict_df = (
            pd.concat(predict_frames, ignore_index=True)
            if len(predict_frames) > 0
        ...
        model = utils_ensemble_model.create_lgbm_model(
            params=lgb_params, n_estimators=args.lgb_n_estimators, random_state=args.random_seed
        )
